...
import { RunningHubConfig, AppConfig, AppsConfig, StorageConfig, RetryConfig, LogConfig, ModelRulesConfig, CloudStorageConfig } from "../types.js";
...
async function loadNewConfig(configDir?: string): Promise<RunningHubConfig> {
  ...
  const config: RunningHubConfig = {
    ...
    modelRules: {
      ...
      defaultLanguage: getString(getObject(serviceConfig, "modelRules"), "defaultLanguage", DEFAULTS.modelRules.defaultLanguage),
    },
    retry: {
      maxRetries: getNumber(getObject(serviceConfig, "retry"), "maxRetries", DEFAULTS.retry.maxRetries),
      maxWaitTime: getNumber(getObject(serviceConfig, "retry"), "maxWaitTime", DEFAULTS.retry.maxWaitTime),
...
function getObject(obj: JsonConfig | null, key: string): JsonConfig | null {
  if (!obj) return null;
  const value = obj[key];
  return typeof value === "object" && value !== null && !Array.isArray(value) ? value as JsonConfig : null;
}

/**
 * 辅助函数：获取存储模式
 */
function getStorageMode(obj: JsonConfig | null, defaultValue: StorageConfig["mode"]): StorageConfig["mode"] {
...
function getCloudConfig(obj: JsonConfig | null): CloudStorageConfig | undefined {
  ...
  if (provider !== "baidu" && provider !== "google" && provider !== "aliyun" && provider !== "aws") {
    return undefined;
  }
...
async function migrateAndLoad(oldPath: string): Promise<RunningHubConfig> {
  ...
  const config: RunningHubConfig = {
    ...
    modelRules: {
      ...
      defaultLanguage: getString(getObject(rawConfig, "modelRules"), "defaultLanguage", DEFAULTS.modelRules.defaultLanguage),
    },
    retry: {
      maxRetries: getNumber(getObject(rawConfig, "retry"), "maxRetries", DEFAULTS.retry.maxRetries),
      maxWaitTime: getNumber(getObject(rawConfig, "retry"), "maxWaitTime", DEFAULTS.retry.maxWaitTime),
...
export function loadConfigSync(configPath?: string): RunningHubConfig {
  ...
  if (legacyPath) {
    ...
    return {
      ...
      apps: (rawConfig.apps as Record<string, AppConfig>) || {},
      appsConfig: { server: {}, user: (rawConfig.apps as Record<string, AppConfig>) || {} },
      modelRules: {
        rules: getModelRules(rawConfig),
        defaultLanguage: getString(getObject(rawConfig, "modelRules"), "defaultLanguage", DEFAULTS.modelRules.defaultLanguage),
      },
