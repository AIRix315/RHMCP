...
export const executeAppTool = {
  ...
  async handler(
    ...
    config: { apps: Record<string, AppConfig>; retry: RetryConfig; storage: { path: string } }
  ) {
    // 1. 解析APP ID
    ...
    // 2. 构建nodeInfoList
    const appConfig = config.apps[Object.keys(config.apps).find(k => 
      config.apps[k].appId === appId
    ) || ''];
