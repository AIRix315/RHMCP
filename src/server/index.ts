...
async function main() {
  ...
  server.registerTool(uploadMediaTool.name, {
    ...
    const result = await uploadMediaTool.handler(args as z.infer<typeof uploadMediaTool.inputSchema>, client);
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  });
  
  // rh_get_app_info
  server.registerTool(getAppInfoTool.name, {
    description: getAppInfoTool.description,
    inputSchema: getAppInfoTool.inputSchema.shape,
  }, async (args, _extra) => {
    const result = await getAppInfoTool.handler(args as z.infer<typeof getAppInfoTool.inputSchema>, client, config);
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  });
  
  // rh_execute_app
  server.registerTool(executeAppTool.name, {
    description: executeAppTool.description,
    inputSchema: executeAppTool.inputSchema.shape,
  }, async (args, _extra) => {
    const result = await executeAppTool.handler(args as z.infer<typeof executeAppTool.inputSchema>, client, config);
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  });
  
  // rh_query_task
  server.registerTool(queryTaskTool.name, {
    description: queryTaskTool.description,
    inputSchema: queryTaskTool.inputSchema.shape,
  }, async (args, _extra) => {
    const result = await queryTaskTool.handler(args as z.infer<typeof queryTaskTool.inputSchema>, client);
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  });
  
  // rh_add_app
  server.registerTool(addAppTool.name, {
    description: addAppTool.description,
    inputSchema: addAppTool.inputSchema.shape,
  }, async (args, _extra) => {
    const result = await addAppTool.handler(args as z.infer<typeof addAppTool.inputSchema>, client, CONFIG_PATH);
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  });
  
  // rh_remove_app
  server.registerTool(removeAppTool.name, {
    description: removeAppTool.description,
    inputSchema: removeAppTool.inputSchema.shape,
  }, async (args, _extra) => {
    const result = await removeAppTool.handler(args as z.infer<typeof removeAppTool.inputSchema>, CONFIG_PATH);
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  });
  
  // rh_update_rules
  server.registerTool(updateRulesTool.name, {
    description: updateRulesTool.description,
    inputSchema: updateRulesTool.inputSchema.shape,
  }, async (args, _extra) => {
    const result = await updateRulesTool.handler(args as z.infer<typeof updateRulesTool.inputSchema>, config);
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  });
  
  // rh_list_rules
  server.registerTool(listRulesTool.name, {
    description: listRulesTool.description,
    inputSchema: listRulesTool.inputSchema.shape,
  }, async (args, _extra) => {
    const result = await listRulesTool.handler(args as z.infer<typeof listRulesTool.inputSchema>, config);
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  });
  
  // rh_validate_config
  server.registerTool(validateConfigTool.name, {
    description: validateConfigTool.description,
    inputSchema: validateConfigTool.inputSchema.shape,
  }, async (args, _extra) => {
    const result = await validateConfigTool.handler(args as z.infer<typeof validateConfigTool.inputSchema>);
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  });
  ...
  server.resource(
    ...
    async (uri: URL) => {
      ...
      return {
        contents: [{ uri: uri.href, mimeType: 'application/json', text: JSON.stringify({ apps }) }]
      };
    }
  );
    ...
    async (uri: URL, variables: Record<string, string | string[]>) => {
      ...
      return {
        contents: [{ 
          ...
          text: JSON.stringify({ ...app, nodeInfoList: result.data?.nodeInfoList }) 
        }]
      };
    }
  );
  ...
  server.resource(
    ...
    async (uri: URL) => {
      ...
      const history = { tasks: [] };
      return {
        contents: [{ uri: uri.href, mimeType: 'application/json', text: JSON.stringify(history) }]
      };
    }
  );
  ...
  server.resource(
    ...
    async (uri: URL) => {
      ...
      const rules = { rules: [] };
      return {
        contents: [{ uri: uri.href, mimeType: 'application/json', text: JSON.stringify(rules) }]
      };
    }
  );
  ...
  server.resource(
    'rh_config',
    'rh://config',
    { description: '查看当前MCP服务配置（隐藏敏感信息）', mimeType: 'application/json' },
    async (uri: URL) => {
      ...
      return {
        contents: [{ uri: uri.href, mimeType: 'application/json', text: JSON.stringify(safeConfig) }]
      };
    }
  );
